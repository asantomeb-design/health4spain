...
- ✅ **GoHighLevel integrado** (jun. 2026 — brief `H4S_BR_1_v2`): API upsert + webhook único con **un POST por servicio**; payload plano (`ciudad` = destino, `origen: web`); upsert Supabase por email/teléfono; textos `*_es` para plantillas. Detalle: **`README.md`** § CRM GHL.
